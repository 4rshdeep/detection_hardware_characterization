...
def run_inference_for_single_image(image, graph):
  with graph.as_default():
    with tf.Session() as sess:
      ...
      # Run inference
      output_dict = sess.run(tensor_dict,options=options,run_metadata=run_metadata,
                             feed_dict={image_tensor: np.expand_dims(image, 0)})

      
      #Storing data from profile in a chrome trace
...
with detection_graph.as_default():
  with tf.Session() as sess:
    ...
    image_tensor = tf.get_default_graph().get_tensor_by_name('image_tensor:0')
    t1 = time.time()
    sess.run(tensor_dict,options=options,run_metadata=run_metadata, feed_dict={image_tensor: np.expand_dims(img, 0)})
    t2 = time.time()
    fst_inference = (t2-t1)
    ...
    for image_path in TEST_IMAGES:
      ...
      # Run inference
      t1 = time.time()
      output_dict = sess.run(tensor_dict,options=options,run_metadata=run_metadata, feed_dict={image_tensor: np.expand_dims(image, 0)})
      t2 = time.time()
      
      #writing profile data in a chrome trace
      fetched_timeline = timeline.Timeline(run_metadata.step_stats)
      chrome_trace = fetched_timeline.generate_chrome_trace_format()
      with open('timeline_01.json', 'w') as f:
        f.write(chrome_trace)
