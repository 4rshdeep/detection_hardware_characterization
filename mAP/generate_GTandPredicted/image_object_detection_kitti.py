...
os.chdir(KITTI_ROOT+"training/image/")
images=os.listdir()
for img in images:
    print (img)
    frame = cv2.imread(KITTI_ROOT+"training/image/"+img)
    org_shape = frame.shape
    frame = imutils.resize(frame, width=400)
    new_shape = frame.shape
    scale= (org_shape[1]/new_shape[1])
   ...
   # print("[INFO] Total Inference time is {}".format(t2 - t1))
	...
	# the bounding box for the object
        ...
        if confidence>args["confidence"]:
            ...
            label = CLASSES[idx]
            if label!="dontcare":
                f.write(label+' '+str(confidence)+' '+str(scale*startX)+' '+str(scale*startY)+' '+str(scale*endX)+' '+str(scale*endY)+"\n")
    f.close()
